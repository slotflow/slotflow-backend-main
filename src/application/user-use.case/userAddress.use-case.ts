...
export class UserCreateAddressUseCase {
    ...
    async execute(payload: CreateAddressRequest): Promise<ApiResponse> {
        try {
            ...
            const address = await this.addressRepositoryImpl.createAddress({ userId: new Types.ObjectId(userId), addressLine, landMark, phone, place, city, district, pincode, state, country, location });
